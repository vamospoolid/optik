...
export default function POSTerminal() {
  ...
  const handleSavePrescriptionInline = async () => {
    ...
    if (isNaN(pdVal) || pdVal < 40 || pdVal > 80) {
      toast.error("Pupil Distance (PD) harus bernilai antara 40mm and 80mm");
      return;
    }
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      resetForm()
      fetchOrders()
      // Dispatch socket custom or dashboard event if active inside same window
      if (typeof window !== "undefined") {
        window.dispatchEvent(new Event("pos_transaction_completed"))
      }
      // Go to history tab
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="bg-slate-100 p-1.5 rounded-2xl flex w-fit gap-2 border border-slate-200/50 mb-6 ml-4 shadow-sm">
          ...
          <TabsTrigger value="history" className="rounded-xl px-6 py-3 font-bold text-xs uppercase tracking-wider data-[state=active]:bg-white data-[state=active]:text-[#1a2b3c] data-[state=active]:shadow-md transition-all flex items-center">
            <Clock className="h-4 w-4 mr-2 text-indigo-505" /> Riwayat Transaksi
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="pos" className="outline-none">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 px-4 items-start">
                                ...
                                {prescriptions.map((pr: any) => {
                                  ...
                                  const getLensTypeBadge = (type: string) => {
                                    switch (type?.toLowerCase()) {
                                      ...
                                      default:
                                        return 'bg-white/10 text-slate-300 border border-white/5'
                                    }
                                  }
                                    ...
                                    <div key={pr.id} className="relative group">
                                      ...
                                      <div className={`absolute -left-[32px] top-[14px] w-4 h-4 rounded-full border-2 transition-all duration-300 z-10 flex items-center justify-center ${
                                        isSelected
                                          ...
                                          : 'bg-slate-900 border-slate-700 group-hover:border-slate-505 group-hover:bg-slate-800'
                                      }`}>
                                      ...
                                      >
                                        <div>
                                          ...
                                          <div className="text-[10px] font-mono text-slate-300 grid grid-cols-2 gap-x-4 mt-1.5">
                                        ...
                                        <div className="flex gap-1.5 self-stretch sm:self-auto justify-end">
                                          <Button
                                            ...
                                            className="h-8 w-8 text-slate-400 hover:text-white border-white/10 bg-white/5 hover:bg-white/10 rounded-lg transition-colors"
                                          >
            ...
            {/* POS-STYLE PATIENT SELECTION (IF NOT SELECTED) */}
            {!patientId && (
              <div className="col-span-12">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  {/* LEFT PANEL: Search + Recent Patients Grid */}
                  <div className="lg:col-span-8 space-y-4">
                    {/* Search Bar - POS Style */}
                    <div className="relative">
                      <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" />
                      <Input
                        ref={patientSearchRef}
                        placeholder="Ketik nama, nomor WhatsApp, atau NIK pasien..."
                        className="pl-12 pr-4 h-14 rounded-2xl bg-white border-slate-200 focus:bg-white focus:border-[#00a39d] focus:ring-2 focus:ring-[#00a39d]/20 text-sm font-bold shadow-sm transition-all"
                        value={patientSearch}
                    ...
                    {/* Patient Results / Recent Grid */}
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          {patientSearch ? `Hasil Pencarian "${patientSearch}"` : "Pilih Pelanggan — Pasien Terdaftar"}
                        </span>
                        <span className="text-[10px] font-bold text-slate-300">
                          {filteredPatients.length} ditemukan
                        </span>
                      </div>

                      {filteredPatients.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                          {(patientSearch ? filteredPatients : patients.slice(0, 9)).map((p) => (
                            <div
                              key={p.id}
                              onClick={() => {
                                  setPatientId(p.id)
                              ...
                              className="bg-white border border-slate-100 rounded-xl p-4 cursor-pointer hover:border-[#00a39d]/50 hover:shadow-md hover:bg-teal-50/30 transition-all group relative overflow-hidden"
                            >
                              <div className="absolute top-0 right-0 w-16 h-16 bg-slate-50 rounded-full -mr-4 -mt-4 group-hover:bg-[#00a39d]/5 transition-colors"></div>
                              <div className="flex items-start gap-3 relative z-10">
                                <div className="h-10 w-10 rounded-xl bg-slate-100 group-hover:bg-[#00a39d]/10 flex items-center justify-center text-slate-400 group-hover:text-[#00a39d] font-extrabold text-sm transition-all shrink-0">
                                  {p.name?.charAt(0).toUpperCase()}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <p className="font-bold text-slate-805 text-sm truncate group-hover:text-[#00a39d] transition-colors">{p.name}</p>
                                  <p className="text-[10px] text-slate-400 font-medium truncate">{p.phone || "No WhatsApp"}</p>
                                  {p.bpjs_number && (
                                    <span className="inline-block mt-1 bg-sky-100 text-sky-700 text-[8px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider">BPJS</span>
                                  )}
                                </div>
                                <ArrowRight className="h-4 w-4 text-slate-200 group-hover:text-[#00a39d] mt-1 shrink-0 transition-colors" />
                              </div>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl p-12 text-center">
                          ...
                          <p className="text-sm font-bold text-slate-400">Pasien tidak ditemukan</p>
                          <p className="text-xs text-slate-300 mt-1">Gunakan form registrasi di samping untuk mendaftarkan pasien baru</p>
                        </div>
                      )}
                    </div>
                  </div>

                  {/* RIGHT PANEL: Quick Registration / Info */}
                  <div className="lg:col-span-4 space-y-4">
                    {/* Registration Card */}
                    <Card className="border-0 shadow-lg rounded-2xl bg-gradient-to-br from-[#1a2b3c] to-slate-900 text-white overflow-hidden">
                      <CardContent className="p-5 space-y-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className="h-8 w-8 rounded-lg bg-white/10 flex items-center justify-center">
                              <UserPlus className="h-4 w-4 text-[#00a39d]" />
                            </div>
                            <span className="text-xs font-bold uppercase tracking-wider">Registrasi Cepat</span>
                          </div>
                        </div>

                        <div className="flex p-0.5 bg-white/10 rounded-lg w-full">
                          <button
                            type="button"
                            onClick={() => { setNewPatientType("umum"); setNewPatientBpjs("") }}
                            className={`flex-1 py-1.5 rounded-md text-[9px] font-extrabold transition-all ${newPatientType === 'umum' ? 'bg-white text-slate-800 shadow-sm' : 'text-white/50'}`}
                          >UMUM</button>
                          <button
                            type="button"
                            onClick={() => setNewPatientType("bpjs")}
                            className={`flex-1 py-1.5 rounded-md text-[9px] font-extrabold transition-all ${newPatientType === 'bpjs' ? 'bg-[#00a39d] text-white shadow-sm' : 'text-white/50'}`}
                          >BPJS</button>
                        </div>

                        <div className="space-y-3">
                          <div className="space-y-1">
                            <Label className="text-[9px] font-bold text-white/40 uppercase">Nama Lengkap *</Label>
                            <Input className="h-9 rounded-lg bg-white/10 border-white/10 text-white font-bold text-xs placeholder:text-white/20 focus:bg-white/15" value={newPatientName} onChange={e => setNewPatientName(e.target.value)} placeholder="Nama pasien..." />
                          </div>
                          <div className="space-y-1">
                            <Label className="text-[9px] font-bold text-white/40 uppercase">No. WhatsApp *</Label>
                            <Input className="h-9 rounded-lg bg-white/10 border-white/10 text-white font-bold text-xs placeholder:text-white/20 focus:bg-white/15" value={newPatientPhone} onChange={e => setNewPatientPhone(e.target.value)} placeholder="08..." />
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                            <div className="space-y-1">
                              <Label className="text-[9px] font-bold text-white/40 uppercase">NIK</Label>
                              <Input className="h-9 rounded-lg bg-white/10 border-white/10 text-white text-xs placeholder:text-white/20 focus:bg-white/15" value={newPatientNik} onChange={e => setNewPatientNik(e.target.value)} placeholder="NIK..." />
                            </div>
                            <div className="space-y-1">
                              <Label className="text-[9px] font-bold text-white/40 uppercase">Tgl Lahir</Label>
                              <Input type="date" className="h-9 rounded-lg bg-white/10 border-white/10 text-white text-xs focus:bg-white/15" value={newPatientBirthDate} onChange={e => setNewPatientBirthDate(e.target.value)} />
                            </div>
                          </div>
                          {newPatientType === 'bpjs' && (
                            <div className="space-y-1 animate-in slide-in-from-top duration-200">
                              <Label className="text-[9px] font-bold text-[#00a39d] uppercase">No. Kartu BPJS *</Label>
                              <Input className="h-9 rounded-lg bg-[#00a39d]/10 border-[#00a39d]/30 text-white font-bold text-xs placeholder:text-white/20" value={newPatientBpjs} onChange={e => setNewPatientBpjs(e.target.value)} placeholder="Kartu BPJS..." />
                            </div>
                          )}
                          <div className="space-y-1">
                            <Label className="text-[9px] font-bold text-white/40 uppercase">Alamat</Label>
                            <Input className="h-9 rounded-lg bg-white/10 border-white/10 text-white text-xs placeholder:text-white/20 focus:bg-white/15" value={newPatientAddress} onChange={e => setNewPatientAddress(e.target.value)} placeholder="Alamat..." />
                          </div>
                        </div>
                        ...
                        <Button 
                          ...
                          className="w-full h-10 bg-[#00a39d] hover:bg-[#008f8a] text-white font-bold text-[10px] uppercase tracking-wider rounded-xl flex gap-2 shadow-lg shadow-[#00a39d]/20"
                        >
                          {registeringPatient ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                          Daftarkan & Mulai Transaksi
                        </Button>
                      </CardContent>
                    </Card>
                    ...
                    <div className="bg-white rounded-xl border border-slate-100 p-4 space-y-3">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Statistik Cepat</span>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="bg-slate-50 rounded-lg p-3 text-center">
                          <p className="text-lg font-extrabold text-slate-800">{patients.length}</p>
                          <p className="text-[9px] font-bold text-slate-400 uppercase">Total Pasien</p>
                        </div>
                        <div className="bg-slate-50 rounded-lg p-3 text-center">
                          <p className="text-lg font-extrabold text-[#00a39d]">{patients.filter(p => p.bpjs_number).length}</p>
                          <p className="text-[9px] font-bold text-slate-400 uppercase">Pasien BPJS</p>
                        </div>
                      </div>
                    </div>
                  </div>
                        ...
                        {(catalogTab === 'all' || catalogTab === 'frame') && (
                          frames
                            ...
                            .map(f => {
                              ...
                              return (
                                <div key={f.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex flex-col justify-between gap-4 hover:border-teal-200 transition-all hover:bg-slate-50/50">
                                  <div className="space-y-1">
                        ...
                        {(catalogTab === 'all' || catalogTab === 'lens') && (
                          lenses
                            ...
                            .map(l => {
                              ...
                              return (
                                <div key={l.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex flex-col justify-between gap-4 hover:border-sky-200 transition-all hover:bg-slate-50/50">
                                  <div className="space-y-1">
                                    <div className="flex justify-between items-start">
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                {item.product_type !== 'service' && (
                                  <div className="flex items-center bg-white border rounded-lg p-0.5 gap-1.5 shadow-sm">
                                    ...
                                    <span className="text-[10px] font-extrabold text-slate-800 w-4 text-center">
                                      {item.qty}
                                    </span>
                                    <button
                                      onClick={() => {
                    ...
                    <CardContent className="p-6 space-y-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex justify-between items-center gap-4 py-1">
                          <div>
                            <span className="text-xs font-bold text-red-500 uppercase tracking-wider block">Potongan Harga / Diskon</span>
                            <span className="text-[10px] text-slate-400 font-medium">Diskon tunai langsung</span>
                          </div>
                          <div className="relative w-36">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-red-500">Rp</span>
                            <Input 
                              className="h-10 rounded-xl bg-red-50/20 border-red-100 text-red-600 font-black text-right pr-4 text-xs focus:ring-red-200" 
                              value={discount} 
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="h-10 flex items-center justify-end px-4 rounded-xl bg-amber-50/50 border border-amber-100 font-bold text-xs text-amber-700 font-mono">
                      ...
                      <div className="pt-2 flex flex-col gap-3">
                        ...
                        <Button 
                          onClick={resetForm}
                          variant="ghost"
                          className="h-11 text-xs font-bold text-slate-400 hover:text-slate-600 rounded-xl"
                        >
                          Reset POS Terminal
                        </Button>
                      </div>
                          ...
                          <TableRow key={order.id} className="hover:bg-slate-50/50 transition-all border-b border-slate-100 group">
                            ...
                            <TableCell>
                              {invoice?.remaining > 0 ? (
                                ...
                                <span className="text-xs font-bold text-emerald-505 flex items-center gap-1"><Check className="h-3 w-3" /> Lunas</span>
                              )}
                            </TableCell>
                            <TableCell>
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="p-6 bg-slate-50 border border-slate-100 rounded-2xl space-y-4">
                   ...
                   {selectedOrder.status === 'cancelled' && (
                     <div className="p-3 bg-red-50 border border-red-100 text-red-650 rounded-xl text-center text-xs font-bold flex items-center justify-center gap-2 mt-4">
